...
impl Decoder {
    ...
    fn run(&mut self, mel: &Tensor) -> Result<String> {
        ...
        let pb = ProgressBar::new(content_frames as u64);
        pb.set_style(ProgressStyle::with_template(
            "{percent}%[{bar:40.green}] {human_pos:>7}/{human_len:7} [{elapsed_precise}>{eta_precise}, {per_sec}]",
        )?.progress_chars("=>-"));
        ...
        while seek < content_frames {
            ...
            seek += segment_size;
            pb.inc(segment_size as u64);
            if dr.no_speech_prob > m::NO_SPEECH_THRESHOLD && dr.avg_logprob < m::LOGPROB_THRESHOLD {
                eprintln!("no speech detected, skipping {seek} {dr:?}");
                continue;
            }
            let segment = Segment {
                start: time_offset,
            ...
            if self.timestamps {
                ...
                let start_seconds = (segment.start * 60. * 60. * 24.).round() as i64;
                let end_seconds =
                    ((segment.start + segment.duration) * 60. * 60. * 24.).round() as i64;

                let start = Duration::seconds(start_seconds);
                let end = Duration::seconds(end_seconds);
                let start_hour = start.num_hours();
                let start_minute = start.num_minutes() - start_hour * 60;
                let start_second = start.num_seconds() - start_minute * 60;
                let start_millis = start.num_milliseconds() - start_second * 1000;
                let end_hour = end.num_hours();
                let end_minute = end.num_minutes() - end_hour * 60;
                let end_second = end.num_seconds() - end_minute * 60;
                let end_millis = end.num_milliseconds() - end_second * 1000;
                number += 1;
                ...
                text_result += &format!(
                    ...
                    format!(
                        "{:02}:{:02}:{:02},{:03}",
                        start_hour, start_minute, start_second, start_millis
                    ),
                    format!(
                        "{:02}:{:02}:{:02},{:03}",
                        end_hour, end_minute, end_second, end_millis
                    )
                );
                ...
                if !tokens_to_decode.is_empty() {
                    let text = self
                        ...
                        .map_err(E::msg)?;
                    if !text.is_empty() {
                        text_result += &format!("  {:.1}s-...: {}\n", prev_timestamp_s, text);
                    }
                    tokens_to_decode.clear()
                }
            } else {
                text_result += &format!("{}", segment.dr.text,);
            }
            if self.verbose {
                text_result += &format!("{seek}: {segment:?}, in {:?}", start.elapsed());
            }
...
pub fn transcribe(args: Args) -> Result<String> {
    ...
    let (config_filename, tokenizer_filename, weights_filename, input) = {
        ...
        let sample = if let Some(input) = args.input {
            ...
            eprintln!("No audio file submitted: Downloading https://huggingface.co/datasets/Narsil/candle_demo/blob/main/samples_jfk.wav");
    ...
    let resampled = tempfile::Builder::new().suffix(".wav").tempfile()?;
    let resampled_path = resampled.path().to_str().unwrap().to_string();
    ...
    let mut input = std::fs::File::open(resampled_path)?;
    let (header, data) = wav::read(&mut input)?;
    eprintln!("loaded wav data: {header:?}");
    ...
    let pcm_data: Vec<_> = data[..data.len() / header.channel_count as usize]
        ...
        .collect();
    eprintln!("pcm data loaded {}", pcm_data.len());
